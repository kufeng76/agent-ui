...
const ReferenceItem: FC<ReferenceItemProps> = ({ reference }) => (
  <div className="bg-background-secondary hover:bg-background-secondary/80 relative flex h-[63px] w-[190px] cursor-default flex-col justify-between overflow-hidden rounded-md p-3 transition-colors">
    <p className="text-primaryAccent text-sm font-medium">{reference.name}</p>
    <p className="text-primaryAccent/60 truncate text-xs">
      {reference.content}
    </p>
  </div>
)
    ...
    <div className="flex flex-col gap-y-9">
      {message.extra_data?.reasoning_steps &&
        message.extra_data.reasoning_steps.length > 0 && (
          <div className="flex items-start gap-4">
            <Tooltip
              delayDuration={0}
              content={<p className="text-primaryAccent">Reasoning</p>}
              side="top"
            >
      ...
      {message.extra_data?.references &&
        message.extra_data.references.length > 0 && (
          <div className="flex items-start gap-4">
            <Tooltip
              delayDuration={0}
              content={<p className="text-primaryAccent">References</p>}
              side="top"
            >
      ...
      {message.tool_calls && message.tool_calls.length > 0 && (
        <div className="flex items-start gap-3">
          <Tooltip
            delayDuration={0}
            content={<p className="text-primaryAccent">Tool Calls</p>}
            side="top"
          >
            <Icon
              type="hammer"
...
const ToolComponent = memo(({ tools }: ToolCallProps) => (
  <div className="bg-accent cursor-default rounded-full px-2 py-1.5 text-xs">
    <p className="font-dmmono text-primaryAccent uppercase">
      {tools.tool_name}
    </p>
  </div>
))
